...
void Plot::addXTick(const wchar_t* _text, float _value) {
    Tick tick;
    tick.text = (wchar_t*)malloc(sizeof(wchar_t) * (wcslen(_text) + 1));
    wcscpy(tick.text, _text);
    tick.value = _value;
    xTicks.push_back(tick);
}

void Plot::addYTick(const wchar_t* _text, float _value) {
    Tick tick;
    tick.text = (wchar_t*)malloc(sizeof(wchar_t) * (wcslen(_text) + 1));
    wcscpy(tick.text, _text);
    tick.value = _value;
...
void Plot::addYTickAlt(const wchar_t* _text, float _value) {
    Tick tick;
    tick.text = (wchar_t*)malloc(sizeof(wchar_t) * (wcslen(_text) + 1));
    wcscpy(tick.text, _text);
    tick.value = _value;
